...
export const resolveCardCssClasses = (opts: CardCssClassOpts): string => {
    ...
    if (!['MusicAlbum', 'MusicArtist', 'Audio'].includes(opts.itemType)) {
...
export const resolveCardImageContainerCssClasses = (opts: { itemType: string, hasCoverImage: boolean, itemName?: string, imgUrl?: string}): string => {
    const classes = ['cardImageContainer'];

    if (opts.hasCoverImage) {
        classes.push('coveredImage');

        if (opts.itemType === 'TvChannel') {
            classes.push('coveredImage-contain');
        }
    }

    if (!opts.imgUrl) {
        classes.push(getDefaultBackgroundClass(opts.itemName));
    }
...
export const resolveCardBoxCssClasses = (opts: { cardLayout: boolean, hasOuterCardFooter: boolean }): string => {
    ...
    if (opts.cardLayout) {
        classes.push('visualCardBox');
    }

    if (opts.hasOuterCardFooter && !opts.cardLayout) {
        classes.push('cardBox-bottompadded');
    }
